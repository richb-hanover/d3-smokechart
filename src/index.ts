...
export const Smokechart = (smokeData: SmokeData, opts?: Partial<SmokechartProps>) => {
  ...
  // and finally...
  //   return smoke(smokeData, opts) // final call of the constructor is at end of the file (must be last)
  ...
  const smoke = (smokeData: SmokeData, opts?: Partial<SmokechartProps>) => {
    if (!(smokeData || !Array.isArray(smokeData))) { throw new Error(`No SmokeData passed in`) }
    smoke.AddProps(opts)      // add any props passed in
    smoke.data(smokeData)     // then process the smokeData
    return smoke
  }

  /**
   * obj.AddProps(Partial<SmokechartProps>)
   * @param newProps set of properties to add to the default SmokechartProps
   * @return smokeProps
   */
  smoke.AddProps = (newProps?: Partial<SmokechartProps>) => {
    if (newProps) Object.assign(smokeProps, newProps)
  }
